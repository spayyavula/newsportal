...
type StoryType = 'reporting' | 'analysis' | 'opinion';
...
const articleSeed: ArticleSeed[] = [
  {
    ...
    contentBlocks: [
      {
        __component: 'editorial.section-block',
      ...
      {
        __component: 'editorial.pull-quote',
        quote:
      ...
      {
        __component: 'editorial.explainer',
...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    for (const topic of topicSeed) {
      const existingTopic = await strapi.documents('api::topic.topic').findFirst({
        filters: { slug: { $eq: topic.slug } },
      });

      const savedTopic = existingTopic?.documentId
        ? await strapi.documents('api::topic.topic').update({
            documentId: existingTopic.documentId,
            data: topic,
          })
        : await strapi.documents('api::topic.topic').create({
            data: topic,
          });

      topicDocumentIds.set(topic.slug, savedTopic.documentId);
    }

    for (const author of authorSeed) {
      const existingAuthor = await strapi.documents('api::author.author').findFirst({
        filters: { slug: { $eq: author.slug } },
      });

      const savedAuthor = existingAuthor?.documentId
      ...
      authorDocumentIds.set(author.slug, savedAuthor.documentId);
    }
